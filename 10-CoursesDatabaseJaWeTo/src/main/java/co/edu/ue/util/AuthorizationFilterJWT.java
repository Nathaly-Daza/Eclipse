...
//public class AuthorizationFilterJWT extends BasicAuthenticationFilter {*/
	...
	/*private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) {
		// Revisar que el token venga en la cabecera de la peticion
		String token = request.getHeader(Tools.ENCABEZADO);
			if (token != null) {
				try {
					// Se procesa el token y se recupera el usuario y los roles.
					Claims claims = Jwts.parserBuilder().setSigningKey(Tools.CLAVE.getBytes()).build()
							.parseClaimsJws(token.replace(Tools.PREFIJO_TOKEN, "")).getBody();
